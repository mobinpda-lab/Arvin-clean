...
#409 and #411 each completed exact-head Fast, Build/APK and Device Smoke before merge. Combined current-main Device Smoke is green; latest current-main Build remains the final combined validation authority.
...
The first Device failure was traced to the Android People smoke trying to tap the longer editor's Save button at the extreme bottom edge. The test now uses Flutter `Scrollable.ensureVisible(..., alignment: 0.5)` so Save is centered in the tappable viewport. Exact-head Build is green; Device Smoke is still executing. Do not merge until Device succeeds and the branch is reconciled to current main without force.
...
### #412 — reusable bulk selection action bar

Fresh Draft lane from current main for #367.

- reusable Task selection action bar
- selected-count display
- visible select-all toggle + clear selection
- trash/category/tag/share callbacks only
- no storage/model/state duplication
- focused widget tests

Keep Draft until exact-head Fast is green and current-main sanity remains clean. Full Build/APK/Device is required before production merge.
...
1. finish #408 fresh full gates, reconcile to current main, and merge safely
...
4. validate/promote #412, then wire #407 + #411 into real Task/Notebook bulk selection/actions
