...
class SettingsPage extends StatefulWidget {
  const SettingsPage({super.key, required this.service, required this.onSettingsChanged, required this.onOpenBackup, this.onStartInteractiveGuide});
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  String _swipeActionLabel(TaskSwipeAction action) => switch (action) { TaskSwipeAction.complete => 'انجام‌شدن', TaskSwipeAction.archive => 'بایگانی', TaskSwipeAction.trash => 'سطل زباله', TaskSwipeAction.moveToToday => 'انتقال به امروز', TaskSwipeAction.none => 'بدون عمل' };
  IconData _swipeActionIcon(TaskSwipeAction action) => switch (action) { TaskSwipeAction.complete => Icons.check_circle_outline, TaskSwipeAction.archive => Icons.archive_outlined, TaskSwipeAction.trash => Icons.delete_outline, TaskSwipeAction.moveToToday => Icons.today_outlined, TaskSwipeAction.none => Icons.block };
  ...
  @override Widget build(BuildContext context) {
    ...
    return Directionality(textDirection: TextDirection.rtl, child: Scaffold(appBar: AppBar(title: const Text('تنظیمات')), body: current == null ? const Center(child: CircularProgressIndicator()) : ListView(padding: const EdgeInsets.all(16), children: [
      const Text('نمایش', style: TextStyle(fontWeight: FontWeight.bold)), const SizedBox(height: 8),
      ...
      const Divider(height: 32), const Text('حرکت کارت‌ها', key: ValueKey('swipe-settings-title'), style: TextStyle(fontWeight: FontWeight.bold)), const SizedBox(height: 6), const Text('عمل کشیدن کارت به راست و چپ را جداگانه تعیین کنید.'), const SizedBox(height: 12),
      ...
      const Divider(height: 24), ListTile(key: const ValueKey('projects-settings-entry'), contentPadding: EdgeInsets.zero, leading: const Icon(Icons.account_tree_outlined), title: const Text('پروژه‌ها'), subtitle: const Text('ساخت، ویرایش، بایگانی و حذف امن پروژه‌ها'), trailing: const Icon(Icons.chevron_left), onTap: _openProjectsManagement),
