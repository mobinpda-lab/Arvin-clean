...
class TaskRepository {
  ...
  Future<List<ArvinTask>> load() async {
    ...
    try {
      ...
      return list.map((e) => ArvinTask.fromJson(Map<String, dynamic>.from(e as Map))).toList();
    } catch (_) { return []; }
  }
...
class _HomePageState extends State<HomePage> {
  ...
  List<ArvinTask> get visible {
    final result = tasks.where((t) {
      ...
      final text = '${t.title} ${t.description} ${t.tags.join(' ')}'.toLowerCase();
  ...
  String _backupJson() => JsonEncoder.withIndent('  ').convert({'format': 'arvin-backup-v1', 'createdAt': DateTime.now().toIso8601String(), 'tasks': tasks.map((e) => e.toJson()).toList()});
  Future<void> _backup() async { await Clipboard.setData(ClipboardData(text: _backupJson())); if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('نسخه پشتیبان ساخته و در کلیپ‌بورد کپی شد.'))); }
  Future<void> _restoreBackup() async {
    ...
    try {
      ...
      if (decoded['format'] != 'arvin-backup-v1' || decoded['tasks'] is! List<dynamic>) throw const FormatException();
      final restored = (decoded['tasks'] as List<dynamic>).map((e) => ArvinTask.fromJson(Map<String, dynamic>.from(e as Map))).toList();
      setState(() { tasks = restored; selected.clear(); selectionMode = false; query = ''; filter = 'فعال'; });
      await _save();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${restored.length} کار بازیابی شد.')));
    } catch (_) { if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('پشتیبان نامعتبر است.'))); }
  }
  void _backupMenu() {
    showModalBottomSheet<void>(context: context, builder: (_) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [ListTile(leading: const Icon(Icons.backup_outlined), title: const Text('ایجاد پشتیبان'), onTap: () { Navigator.pop(context); _backup(); }), ListTile(leading: const Icon(Icons.restore_outlined), title: const Text('بازیابی پشتیبان'), onTap: () { Navigator.pop(context); _restoreBackup(); })])));
  }
  Widget _taskCard(ArvinTask t) {
    final late = _overdue(t);
    return Dismissible(key: ValueKey(t.id), direction: selectionMode ? DismissDirection.none : DismissDirection.endToStart, confirmDismiss: (_) async { if (t.trashed) { await _deleteForever(t); } else { setState(() => t.trashed = true); await _save(); } return true; }, background: Container(alignment: Alignment.centerLeft, padding: const EdgeInsets.symmetric(horizontal: 20), color: Theme.of(context).colorScheme.errorContainer, child: const Icon(Icons.delete_outline)), child: Card(child: ListTile(
      ...
      subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [if (t.description.isNotEmpty) Text(t.description), if (t.tags.isNotEmpty) Wrap(spacing: 4, children: t.tags.map<Widget>((tag) => Chip(label: Text(tag), visualDensity: VisualDensity.compact)).toList()), if (t.followUpDate != null) Text('پیگیری: ${_date(t.followUpDate!)}${late ? ' • عقب‌افتاده' : ''}'), if (t.trashed) TextButton(onPressed: () => _restore(t), child: const Text('بازگردانی'))]),
    )));
  }
...
class _Stat extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(6), child: Column(children: [Icon(icon, size: 20), Text('$value', style: const TextStyle(fontWeight: FontWeight.bold)), Text(label, style: const TextStyle(fontSize: 10))])));
}
...
class _TaskDialogState extends State<TaskDialog> {
  late final TextEditingController title, desc, tag;
  DateTime? followUpDate;
