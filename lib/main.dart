...
class _HomePageState extends State<HomePage> {
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: loading ? const Center(child: CircularProgressIndicator()) : Column(children: [
      ...
      SizedBox(height: 52, child: ListView(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 12), children: ['فعال', 'بایگانی', 'سطل زباله'].map((f) => Padding(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 8), child: ChoiceChip(label: Text(f), selected: filter == f, onSelected: (_) => setState(() => filter = f))).toList())),
