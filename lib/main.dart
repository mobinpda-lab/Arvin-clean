...
class TaskRepository {
  ...
  Future<List<ArvinTask>> load() async { final p = await SharedPreferences.getInstance(); final raw = p.getString(key); if (raw == null || raw.isEmpty) return []; try { final data = jsonDecode(raw) as List<dynamic>; return data.map((e) => ArvinTask.fromJson(Map<String, dynamic>.from(e as Map))).toList(); } catch (_) { return []; } }
  Future<void> save(List<ArvinTask> tasks) async { final p = await SharedPreferences.getInstance(); await p.setString(key, jsonEncode(tasks.map((e) => e.toJson()).toList())); }
}
...
class _HomePageState extends State<HomePage> {
  ...
  List<ArvinTask> get visible {
    final result = tasks.where((t) {
      ...
      return query.trim().isEmpty || '${t.title} ${t.description} ${t.tags.join(' ')}'.toLowerCase().contains(query.trim().toLowerCase());
    }).toList();
  ...
  Future<void> _toggle(ArvinTask t) async { setState(() => t.completed = !t.completed); await _save(); }
  Future<void> _backup() async { final text = jsonEncode(tasks.map((t) => t.toJson()).toList()); await Clipboard.setData(ClipboardData(text: text)); if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('نسخه پشتیبان در کلیپ‌بورد کپی شد'))); }
  Future<void> _restoreFromClipboard() async { final data = await Clipboard.getData(Clipboard.kTextPlain); final text = data?.text; if (text == null || text.trim().isEmpty) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('اطلاعات پشتیبان در کلیپ‌بورد وجود ندارد'))); return; } try { final list = (jsonDecode(text) as List<dynamic>).map((e) => ArvinTask.fromJson(Map<String, dynamic>.from(e as Map))).toList(); setState(() => tasks = list); await _save(); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('اطلاعات با موفقیت بازیابی شد'))); } catch (_) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('فرمت پشتیبان نامعتبر است'))); } }
  Future<void> _backupMenu() async { await showModalBottomSheet<void>(context: context, builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [ListTile(leading: const Icon(Icons.copy), title: const Text('ایجاد Backup'), onTap: () { Navigator.pop(ctx); _backup(); }), ListTile(leading: const Icon(Icons.restore), title: const Text('Restore از کلیپ‌بورد'), onTap: () { Navigator.pop(ctx); _restoreFromClipboard(); })]))); }

  Widget _taskCard(ArvinTask t) {
    final late = _overdue(t);
    return Dismissible(
      key: ValueKey(t.id), direction: selectionMode ? DismissDirection.none : DismissDirection.endToStart,
      confirmDismiss: (_) async { if (t.trashed) { await _deleteForever(t); } else { setState(() => t.trashed = true); await _save(); } return true; },
      ...
      child: Card(child: ListTile(
        onLongPress: () => setState(() { selectionMode = true; selected.add(t.id); }),
        onTap: selectionMode ? () => setState(() { selected.contains(t.id) ? selected.remove(t.id) : selected.add(t.id); }) : () => _edit(t),
        leading: selectionMode ? Checkbox(value: selected.contains(t.id), onChanged: (_) => setState(() { selected.contains(t.id) ? selected.remove(t.id) : selected.add(t.id); })) : IconButton(onPressed: () => _toggle(t), icon: Icon(t.completed ? Icons.check_circle : late ? Icons.warning_amber : Icons.radio_button_unchecked)),
        ...
        subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (t.tags.isNotEmpty) Wrap(spacing: 4, children: t.tags.map<Widget>((tag) => Chip(label: Text(tag), visualDensity: VisualDensity.compact)).toList()),
          if (t.followUpDate != null) Text('پیگیری: ${_date(t.followUpDate!)}${late ? '  •  عقب‌افتاده' : ''}'),
          if (t.trashed) TextButton(onPressed: () => _restore(t), child: const Text('بازگردانی')),
        ]),
      )),
    );
  }
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: loading ? const Center(child: CircularProgressIndicator()) : Column(children: [
      ...
      Expanded(child: visible.isEmpty ? Center(child: Text(filter == 'سطل زباله' ? 'سطل زباله خالی است' : 'کاری برای نمایش وجود ندارد')) : ListView.separated(padding: const EdgeInsets.fromLTRB(16, 8, 16, 96), itemCount: visible.length, separatorBuilder: (_, __) => const SizedBox(height: 8), itemBuilder: (_, i) => _taskCard(visible[i]))),
    ]),
...
class _TaskDialogState extends State<TaskDialog> {
  ...
  @override void initState() { super.initState(); final t = widget.task; title = TextEditingController(text: t?.title ?? ''); desc = TextEditingController(text: t?.description ?? ''); tag = TextEditingController(); followUpDate = t?.followUpDate; tags = List<String>.of(t?.tags ?? const []); }
  ...
  void _addTag() { final v = tag.text.trim(); if (v.isEmpty) return; setState(() { tags.add(v); tag.clear(); }); }
  void _submit() { final v = title.text.trim(); if (v.isEmpty) return; Navigator.pop(context, ArvinTask(id: widget.task?.id ?? DateTime.now().microsecondsSinceEpoch.toString(), title: v, description: desc.text.trim(), followUpDate: followUpDate, tags: List<String>.of(tags), archived: widget.task?.archived ?? false, trashed: widget.task?.trashed ?? false, completed: widget.task?.completed ?? false)); }
  @override Widget build(BuildContext context) => AlertDialog(title: Text(widget.task == null ? 'کار جدید' : 'ویرایش کار'), content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [TextField(controller: title, autofocus: true, decoration: const InputDecoration(labelText: 'عنوان کار')), const SizedBox(height: 12), TextField(controller: desc, minLines: 3, maxLines: 5, decoration: const InputDecoration(labelText: 'توضیحات')), const SizedBox(height: 12), Row(children: [Expanded(child: TextField(controller: tag, onSubmitted: (_) => _addTag(), decoration: const InputDecoration(labelText: 'تگ'))), IconButton(onPressed: _addTag, icon: const Icon(Icons.add))]), Wrap(spacing: 4, children: tags.map<Widget>((v) => InputChip(label: Text(v), onDeleted: () => setState(() => tags.remove(v))).toList()), ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.event_outlined), title: Text(followUpDate == null ? 'تاریخ پیگیری انتخاب نشده' : 'تاریخ: ${followUpDate!.year}/${followUpDate!.month}/${followUpDate!.day}'), trailing: TextButton(onPressed: _pickDate, child: const Text('انتخاب'))])), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('لغو')), FilledButton(onPressed: _submit, child: const Text('ذخیره'))]);
}
