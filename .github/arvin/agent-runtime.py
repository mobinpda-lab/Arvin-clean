...
def validate_diff(diff):
    ...
    for i in range(len(starts)-1):
        ...
        minus = next((j for j,l in enumerate(section) if l.startswith("--- ")), None)
        plus = next((j for j,l in enumerate(section) if l.startswith("+++ ")), None)
        hunk = next((j for j,l in enumerate(section) if l.startswith("@@ ")), None)
...
Repository context below is secret-safe and bounded. Use it to understand the real code before changing it.
{context}
...
def main():
    ...
    expected = os.getenv("ARVIN_EXPECTED_MAIN_SHA", "").strip()
    repo = os.environ["GITHUB_REPOSITORY"]
    ...
    if "arvin-auto" not in labels:
        raise RuntimeError("Issue is not opted into Arvin autonomous execution")
    lease(issue_number)
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        try:
            ...
            failure = str(exc); continue
        if not diff:
            failure = "AI returned no actionable diff"; continue
        ok, message = apply_diff(diff)
        if not ok:
            run(["git", "reset", "--hard", "HEAD"], check=False)
            failure = f"Patch rejected: {message}"; continue
        passed, evidence = project_test()
        if passed:
            print("ARVIN_CODE_WORKER_PASS"); return 0
