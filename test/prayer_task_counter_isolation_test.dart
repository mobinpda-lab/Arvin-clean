  ...
  test('prayer state never changes canonical Task storage or Task counters', () async {
    ...
    final tasks = <Task>[
      Task(
        ...
        followUpDate: DateTime(2026, 9, 15, 10),
      ),
      Task(
        id: 'overdue-open',
        title: 'کار عقب‌افتاده',
        dueDate: DateTime(2026, 9, 14, 9),
      ),
      Task(
        id: 'completed',
        title: 'کار انجام‌شده',
    ...
    SharedPreferences.setMockInitialValues(<String, Object>{
      TaskStore.key: rawTasks,
    });

    final store = TaskStore();
    final before = await store.load();
    final beforeToday = const HomeTodayProjection().select(before, now: now);
    final beforeDueToday = const TaskDueScopeService().project(
      before,
      now: now,
      scope: TaskDueScope.today,
    );
    final beforeOverdue = const TaskDueScopeService().project(
      before,
      now: now,
    ...
    expect(after.map((task) => task.toJson()).toList(),
        before.map((task) => task.toJson()).toList());
    expect(after.length, before.length);
    expect(
      const HomeTodayProjection().select(after, now: now).map((task) => task.id),
      beforeToday.map((task) => task.id),
    );
    expect(
      const TaskDueScopeService()
          .project(after, now: now, scope: TaskDueScope.today)
          .map((task) => task.id),
      beforeDueToday.map((task) => task.id),
    );
    expect(
      const TaskDueScopeService()
          .project(after, now: now, scope: TaskDueScope.overdue)
          .map((task) => task.id),
      beforeOverdue.map((task) => task.id),
    );
