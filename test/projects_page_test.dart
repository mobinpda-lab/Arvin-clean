...
void main() {
  testWidgets('renders colored project cards and blocks deleting non-empty project', (tester) async {
    var changed = <ProjectPlan>[];
    final projects = [
      ProjectPlan(id: 'work', title: 'کاری', colorValue: 0xFF2F80ED, itemIds: const ['task-1']),
      ProjectPlan(id: 'personal', title: 'شخصی', colorValue: 0xFF27AE60),
    ];

    await tester.pumpWidget(MaterialApp(
      home: ProjectsPage(projects: projects, onChanged: (value) => changed = value),
    ));
  ...
  testWidgets('can add a project with title and selected color', (tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      home: ProjectsPage(projects: const [], onChanged: (value) => changed = value),
    ));

    await tester.tap(find.byKey(const ValueKey('projects-add')));
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const ValueKey('project-title-input')), 'آروین');
