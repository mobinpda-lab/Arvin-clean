...
void main() {
  ...
  test(
    'legacy JSON migrates to SQL with ids, relations, and unknown fields intact',
    () async {
      ...
      try {
        ...
        final store = TaskStore(executor: db);
        ...
        final envelope =
            jsonDecode(rows.single['legacy_payload_json'] as String)
  ...
  test(
    'canonical SQL remains the source after legacy preference changes',
    () async {
      final db = NativeDatabase.memory();
      try {
        final prefs = await SharedPreferences.getInstance();
