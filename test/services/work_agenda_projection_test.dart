...
void main() {
  ...
  test('inclusive range groups by local day and keeps deterministic order', () {
    final tasks = [
      Task(id: 'b', title: 'ب', dueDate: DateTime(2026, 8, 29, 9)),
      Task(id: 'a', title: 'الف', reminderDate: DateTime(2026, 8, 28, 10)),
      Task(id: 'c', title: 'ج', dueDate: DateTime(2026, 8, 30, 8)),
      Task(id: 'aa', title: 'الف دوم', dueDate: DateTime(2026, 8, 28, 10)),
    ];
    ...
    expect(result.expand((day) => day.items).any((item) => item.taskId == 'c'), isFalse);
  });
  ...
  test('same Task may appear on different range days for different work events', () {
    ...
    expect(result[0].items.single.events.single.kind, WorkAgendaEventKind.taskDue);
    expect(result[1].items.single.taskId, 'task');
    expect(result[1].items.single.events.single.kind, WorkAgendaEventKind.taskReminder);
  });
  ...
  test('archived and trashed are excluded while completed work stays truthful', () {
    ...
    final result = projection.forDay(
      [
        Task(id: 'archived', title: 'بایگانی', dueDate: DateTime(2026, 8, 28, 8), archived: true),
        Task(id: 'trashed', title: 'زباله', reminderDate: DateTime(2026, 8, 28, 9), trashed: true),
        Task(id: 'completed', title: 'انجام شده', dueDate: DateTime(2026, 8, 28, 10), completed: true),
      ],
      day: day,
    );
  ...
  test('older nextFollowUp is superseded by latest real FollowUp', () {
    final task = Task(
      ...
      followUps: [
        ...
        FollowUp(id: 'latest', dateTime: DateTime(2026, 8, 27)),
      ],
    );
