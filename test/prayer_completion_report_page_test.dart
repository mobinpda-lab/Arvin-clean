...
void main() {
  ...
  testWidgets('report shows today counts and missed prayer list', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: PrayerCompletionReportPage(
          store: store,
    ...
    expect(
      find.descendant(
        of: find.byKey(const ValueKey('prayer-report-completed')),
    ...
    expect(
      find.descendant(
        of: find.byKey(const ValueKey('prayer-report-missed')),
        matching: find.text('۱'),
      ),
      findsOneWidget,
    );
  ...
  testWidgets('range scope exposes Persian date controls', (tester) async {
    ...
    expect(
      find.byKey(const ValueKey('prayer-report-range-start')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey('prayer-report-range-end')),
      findsOneWidget,
    );
  });
}
