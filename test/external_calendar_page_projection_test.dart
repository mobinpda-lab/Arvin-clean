...
class _SettingsService extends AppSettingsService {
  ...
  @override
  Future<AppSettings> load() async => AppSettings(themeMode: ThemeMode.system, usePersianDate: true, fontFamily: null, calendarIntegration: integration);
}
...
class _CalendarBridge extends SystemCalendarBridge {
  ...
  @override
  Future<bool> requestReadPermission() async { requestedPermission = true; return permissionGranted; }
  @override
  Future<List<DeviceCalendarEvent>> listDeviceCalendarEvents({required Iterable<String> calendarIds, required DateTime start, required DateTime end}) async {
    listedEvents = true;
...
class _LinkStore extends ExternalCalendarLinkStore {
  @override
  Future<List<ExternalCalendarEventLink>> load() async => const <ExternalCalendarEventLink>[];
}

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  testWidgets('shows selected external device events without prompting permission', (tester) async {
  ...
  testWidgets('permission denial fails closed and keeps calendar usable', (tester) async {
    ...
    await tester.pumpWidget(MaterialApp(home: OfficialCalendarPage(service: const OfficialCalendarReminderService([]), years: <int>[now.year], initialSelectedDay: now, settingsService: _SettingsService(const CalendarIntegrationSettings(enabled: true, showExternalEvents: true, visibleCalendarIds: {'calendar-7'})), calendarBridge: bridge, externalLinkStore: _LinkStore())));
    await tester.pump(const Duration(milliseconds: 250));
